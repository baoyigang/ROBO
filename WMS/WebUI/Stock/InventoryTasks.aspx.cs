    ...
    protected void btnAdd_Click(object sender, EventArgs e)
    {
        ...
        for (int i = 0; i < this.GridView1.Rows.Count; i++)
        {
            ...
            if (cb != null && cb.Checked)
            {
                ...
                if (State == 0)
                {
                    JScript.ShowMessage(this.UpdatePanel1, GridView1.Rows[i].Cells[2].Text + "单号还未审核不能作业，请审核后，再进行盘库作业。");
                    BindDataSub(this.hdnRowValue.Value);
                    return;
                }
                if (State > 1)
                {
                    JScript.ShowMessage(this.UpdatePanel1, GridView1.Rows[i].Cells[2].Text + "单号已经作业，不能再进行盘库作业。");
                    BindDataSub(this.hdnRowValue.Value);
                    return;
                }
