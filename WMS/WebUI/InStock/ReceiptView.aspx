<%@ Page Language="C#" AutoEventWireup="true" CodeFile="ReceiptView.aspx.cs" Inherits="WebUI_InStock_ReceiptView" %>
...
<head id="Head1" runat="server">
    ...
    <script type="text/javascript">
        ...
        function ShowPdfInfo(PdfName) {
//             var src1 = "../../RptFiles/PDF/" + PdfName;
//            var diag = new Dialog();
//            diag.Width = 800;
//            diag.Height = 450;
//            diag.Title = "打印";
//            diag.InnerHtml = '<div style="text-align:center;"><embed width="800" height="600" src=" + src1 + "></embed></div>'
//            diag.show();

            location.href = "../Query/StockPrint.aspx?FormID=" + FormID + "&BillID=" + $('#txtID').val() + "&PdfName=" + PdfName;

            closeinfo();
            var product = document.getElementById("productinfo");
            var src1 = "../../RptFiles/PDF/" + PdfName;
            $('#divPrint1').empty();
            $('#divPrint1').append("<embed width='800' height='450' src='" + src1 + "'></embed>");
            showinfo();
        }
        function showinfo() {
            var product = document.getElementById("productinfo");
            var objtop = (document.documentElement.clientHeight - 30 - 450) / 2; //获得窗口的垂直位置;
            var objleft = (document.documentElement.clientWidth - 10 - 800) / 2; //获得窗口的水平位置;
            product.style.top = parseFloat(objtop) + "px";
            ...
            <ContentTemplate>
                 <table  style="width: 100%; height: 25px;" class="OperationBar">
                    <tr>
                        ...
                        <td align="right">
                            <asp:Button ID="btnUpERP" runat="server" Text="上传" CssClass="ButtonUpLoad" Visible="false" onclick="btnUpERP_Click" />
                            <asp:Button ID="btnAdd" runat="server" Text="新增" CssClass="ButtonCreate" OnClientClick="return Add();"  />
                            <asp:Button ID="btnDelete" runat="server" Text="刪除" CssClass="ButtonDel" OnClientClick="return ViewDelete();" onclick="btnDelete_Click"  />
                            <asp:Button ID="btnEdit" runat="server" Text="修改" CssClass="ButtonModify" OnClientClick="return ViewEdit();" />
                            <asp:Button ID="btnCheck" runat="server" Text="审核" CssClass="ButtonAudit" onclick="btnCheck_Click"  />
                            <asp:Button ID="btnPrint" runat="server" Text="打印" CssClass="ButtonPrint" onclick="btnPrint_Click" />
                            <asp:Button ID="btnTask" runat="server" Text="入库作业" CssClass="ButtonTask" Visible="false" onclick="btnTask_Click"  />
                            <asp:Button ID="btnUnTask" runat="server" Text="取消作业" CssClass="ButtonUnTask" Visible="false" onclick="btnUnTask_Click"  />
                ...
                <table  class=" maintable" style="width:100%; height:25px" > 
                    <tr>
                        ...
                        <td style="width:9%">
                    &nbsp;<asp:TextBox ID="txtTotalQty" runat="server" CssClass="TextRead"  ReadOnly="True" Width="90%" style="text-align:right">
                                    </asp:TextBox>
