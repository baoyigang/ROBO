...
public partial class WebUI_InStock_ReceiptView : BasePage
{
    ...
    private void BindDropDownList()
    {
        ...
        DataRow drNew = dtFactory.NewRow();
        drNew["FactoryID"] = "";
        drNew["FactoryName"] = "";
        dtFactory.Rows.InsertAt(drNew, 0);
    ...
    protected void btnDelete_Click(object sender, EventArgs e)
    {
        ...
        if (Count > 0)
        {
            JScript.ShowMessage(this.updatePanel, "该入库单号已被其它单据使用，请调整后再删除！");
            return;
        }
    ...
    {
        BindData(bll.GetRecord("F", TableName, "BillID like 'RS%'", PrimaryKey, this.txtID.Text));
    }
    protected void btnPre_Click(object sender, EventArgs e)
    {
        BindData(bll.GetRecord("P", TableName, "BillID like 'RS%'", PrimaryKey, this.txtID.Text));
    }
    protected void btnNext_Click(object sender, EventArgs e)
    {
        BindData(bll.GetRecord("N", TableName, "BillID like 'RS%'", PrimaryKey, this.txtID.Text));
    }
    protected void btnLast_Click(object sender, EventArgs e)
    {
        BindData(bll.GetRecord("L", TableName, "BillID like 'RS%'", PrimaryKey, this.txtID.Text));
    }
    #endregion
    ...
    protected void btnTask_Click(object sender, EventArgs e)
    {
        ...
        if (State == 0)
        {
            JScript.ShowMessage(this.updatePanel, this.txtID.Text + "单号还未审核不能作业，请审核后，再进行入库作业。");
            return;
        }
        if (State > 1)
        {
            JScript.ShowMessage(this.updatePanel, this.txtID.Text + "单号已经作业，不能再进行入库作业。");
            return;
        }
        try
        {
            bll.ExecNonQueryTran("WMS.SpInstockTask", new DataParameter[] { new DataParameter("@strWhere", this.txtID.Text), new DataParameter("@UserName", Session["EmployeeCode"].ToString()) });
            AddOperateLog("入库单", "入库作业单号：" + this.txtID.Text);
            DataTable dt = bll.FillDataTable("WMS.SelectBillMaster", new DataParameter[] { new DataParameter("{0}", string.Format("BillID='{0}'", strID)) });
            BindData(dt);
        }
        catch (Exception ex)
    ...
    protected void btnPrint_Click(object sender, EventArgs e)
    {
        ...
        {
            report.Load(Path + @"ReceiptView.frx");

            report.RegisterData(dtNew, "ReceiptView");

            report.Prepare();
            //report.FinishReport += new EventHandler(report_FinishReport);
            string FileName = Path + @"PDF\ReceiptView_" + this.txtID.Text + ".pdf";
            FastReport.Export.Pdf.PDFExport pdfExport = new FastReport.Export.Pdf.PDFExport();

            report.Export(pdfExport, FileName);

            FileName = "ReceiptView_" + this.txtID.Text + ".pdf";
            Response.Redirect("../Query/StockPrint.aspx?FormID=" + FormID + "&BillID=" + this.txtID.Text + "&PdfName=" + FileName + "&SubModuleCode=" + SubModuleCode + "&SqlCmd=" + SqlCmd + "&PagePath=InStock");
