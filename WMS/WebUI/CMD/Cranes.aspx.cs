...
public partial class WebUI_CMD_Cranes : BasePage
{
    ...
    protected void btnDeletet_Click(object sender, EventArgs e)
    {
        ...
        for (int i = 0; i < this.GridView1.Rows.Count; i++)
        {
            ...
            if (cb != null && cb.Checked)
            {
                ...
                //判断能否删除
                int Count = bll.GetRowCount("VUsed_CMD_TrainType", string.Format("TypeCode='{0}'", hk.Text));
                if (Count > 0)
                {
                    JScript.ShowMessage(this.UpdatePanel1, GridView1.Rows[i].Cells[2].Text + "车型信息被其它单据使用，请调整后再删除！");
                    return;
                }
