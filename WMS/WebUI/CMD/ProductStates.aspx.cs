...
public partial class WebUI_CMD_ProductStates : BasePage
{
    ...
    protected void btnDeletet_Click(object sender, EventArgs e)
    {
        ...
        for (int i = 0; i < this.GridView1.Rows.Count; i++)
        {
            ...
            if (cb != null && cb.Checked)
            {
                ...
                //判断能否删除
                int Count = bll.GetRowCount("VUsed_CMD_ProductState", string.Format("StateNo='{0}'", hk.Text));
                if (Count > 0)
                {
                    JScript.ShowMessage(this.UpdatePanel1, GridView1.Rows[i].Cells[2].Text + " 状态被其它单据使用，请调整后再删除！");
                    return;
                }
        ...
        bll.ExecNonQuery("Cmd.DeleteProductState", new DataParameter[] { new DataParameter("{0}", strColorCode) });
        AddOperateLog("产品状态", "删除单号：" + strColorCode.Replace("'-1',", "").Replace(",'-1'", ""));
        SetBtnEnabled(int.Parse(ViewState["CurrentPage"].ToString()), SqlCmd, ViewState["filter"].ToString(), pageSize, GridView1, btnFirst, btnPre, btnNext, btnLast, btnToPage, lblCurrentPage, this.UpdatePanel1);
